...
public class StandardGameLogic extends BaseGameLogic {
    ...
    protected EventResult onPlayerBreakBlock(ServerPlayer player, ServerLevel world, BlockPos blockPos) {
        ...
        if (playerData != null && monument != null) {
            if (monument.teamData == playerData.teamData) {
                player.displayClientMessage(DtmUtil.getText("message", "cant_break_own").withStyle(ChatFormatting.RED), true);
                ...
                this.gameSpace.getPlayers().sendPacket(new ClientboundExplodePacket( Vec3.atCenterOf(blockPos), 4.0f ,0, Optional.empty(), ParticleTypes.EXPLOSION_EMITTER, SoundEvents.GENERIC_EXPLODE, WeightedList.of()));
    ...
    protected boolean checkIfShouldEnd() {
        ...
        for (var teamData : this.teams) {
            ...
            if (!teamData.aliveMonuments.isEmpty() && players > 0) {
                aliveTeams += 1;
            }
        }
    ...
    protected void buildSidebar() {
        ...
        if (this.sidebarTeams == null) {
            ...
            this.currentSidebarTeam = this.sidebarTeams.getFirst();
        }
